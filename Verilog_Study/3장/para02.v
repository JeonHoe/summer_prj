...
reg [3:0] port_id [0:7]; // 8 4 비트 port_id 레지스터 변수 배열.
...
// 종종 디지털 시뮬레이션에서 레지스터 파일, RAMs, 그리고 ROMs과 같은 모델이 필요.
// Verilog에서 메모리는 단순히 레지스터의 배열로 만들어짐.
// 배열의 각 원소는 워드. 각 워드는 1또는 여러 비트가 될 수 있다.

reg mem1bit [0:1023]; // 메모리 mem1bit는 1K 1 비트 워드를 가진다.
reg [7:0] membyte [0:1023] // 메모리 membyte는 1K 8 비트 워드(바이트)를 가진다.
membyte [51] //  주소가 511dls 1바이트 워드를 나타낸다.

// 파라미터

// Verilog는 키워드 Parameter로 모듈 내에서 상수를 정의할 수 있다.
// 파라미터는 변수로 사용 불가능.
// 각 모듈 인스턴스의 파라미터는 컴파일 시 개별적으로 대치.
// 파라미터의 타입과 크기는 정의될 수 있다.

parameter port_id = 5; // port_id를 정의한다.
parameter cache_line_width = 256; // 캐쉬 줄의 폭을 정의한 상수.
parameter signed [15:0] WIDTH; // 파라미터 WIDTH에 대한 부호와 범위 고정

// 파라미터를 사요해서 모듈을 작성한다. 숫자를 직접 사용하는 것은 피한다.
// 모듈 파생시 또는 defparam 문장을 사용함으로써 파라미터를 바꿀 수 있다.
// Verilog 로컬 파라미터(localparam)는 defparam 문장 혹은 정렬된 혹은 이름 지어진 파라미터 값할당에 의해 직접적 조작 불가능.
...
// 문자열은 reg에 저장될 수 있다.
// 레지스터 변수의 폭은 문자열을 저장할 수 있을 만큼 커야 한다.
// 문자열의 각 문자는 8 비트(1 바이트)를 차지한다.
// 레지스터의 폭이 문자열의 크기보다 크면, Verilog는 문자열의 왼쪽 비트를 0으로 채운다.
// 반대로 폭의 크기가 문자열보다 작다면, Verliog는 문자열의 왼쪽 비트들을 무시한다.
// 문자열 선언시 레지스터의 폭을 필요한 것보다 크게 하는 것이 안전.

reg [8*18:1] string_value; // 18 바이트의 폭을 갖는 변수 선언.
initial
    string_value = "Hello Verilog world"; // 변수 문자열 저장.

// 특수 문자들
// 이스케이프 문자
// \n -> 개행, \t -> 탭, %% -> %, \\ -> \, \" -> ", \ooo -> 1-3 8진수로 쓰여진 문자
